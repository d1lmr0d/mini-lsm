...
impl LsmStorageInner {
    ...
    pub fn get(&self, _key: &[u8]) -> Result<Option<Bytes>> {
        ...
        for l0_sstable in snapshot.l0_sstables.iter() {
            ...
            let l0_iter = SsTableIterator::create_and_seek_to_key(l0_sst, key)?;
            l0_iters.push(Box::new(l0_iter));
        }
        let l0_iter = MergeIterator::create(l0_iters);
        if l0_iter.is_valid() && key == l0_iter.key() {
        ...
        if l1_iter.is_valid() && key == l1_iter.key() {
    ...
    pub fn put(&self, _key: &[u8], _value: &[u8]) -> Result<()> {
        let need_freeze = {
            ...
            state.memtable.approximate_size() >= self.options.num_memtable_limit
        };
        if need_freeze {
            let state = self.state.read();
    ...
    pub fn delete(&self, _key: &[u8]) -> Result<()> {
        let need_freeze = {
            let state = self.state.read();
            let tombstone: &[u8] = &[];
            state.memtable.put(_key, tombstone)?;
            state.memtable.approximate_size() >= self.options.num_memtable_limit
        };
        if need_freeze {
            let state = self.state.read();
    ...
    pub fn force_flush_next_imm_memtable(&self) -> Result<()> {
        ...
        let snapshot = {
            let state = self.state.read();
            memtable_to_flush = match state.imm_memtables.last() {
                Some(m) => m.clone(),
                None => return Ok(()),
            };
            Arc::clone(&state)
        ...
        let sst = Arc::new(sst_builder.build(id, Some(self.block_cache.clone()), path)?);
        {
    ...
    pub fn scan(
        ...
        for l0_sstable in snapshot.l0_sstables.iter() {
            ...
            let l0_sst_iter = match _lower {
                Bound::Included(key) => {
                    SsTableIterator::create_and_seek_to_key(l0_sst, KeySlice::from_slice(key))?
                }
                Bound::Excluded(key) => {
                    let mut iter =
                        SsTableIterator::create_and_seek_to_key(l0_sst, KeySlice::from_slice(key))?;
                    while iter.is_valid() && iter.key() == KeySlice::from_slice(key) {
                        iter.next()?;
                    }
                    iter
                }
                Bound::Unbounded => SsTableIterator::create_and_seek_to_first(l0_sst)?,
            };
            l0_sst_iters.push(Box::new(l0_sst_iter));
        }
        let mut mem_iters = Vec::new();
        for memtable in snapshot.imm_memtables.iter() {
            mem_iters.push(Box::new(memtable.scan(_lower, _upper)));
        }
        mem_iters.insert(0, Box::new(snapshot.memtable.scan(_lower, _upper)));
        let mut l1_ssts = Vec::new();
        for sst_id in snapshot.levels[0].1.iter() {
            l1_ssts.push(snapshot.sstables[sst_id].clone());
        }
        let l1_iter = SstConcatIterator::create_and_seek_to_first(l1_ssts)?;
        let l_iters = TwoMergeIterator::create(l1_iter, MergeIterator::create(l0_sst_iters))?;
        let two_merge_iter = TwoMergeIterator::create(MergeIterator::create(mem_iters), l_iters)?;
        let lsm_iter = LsmIterator::new(two_merge_iter, _upper.map(Bytes::copy_from_slice));
        Ok(FusedIterator::new(lsm_iter.unwrap()))
    }
}

fn key_within(first_key: KeySlice, last_key: KeySlice, key: KeySlice) -> bool {
...
fn range_overlap(
    ...
    let lower_ok = match lower {
        Bound::Included(k) => last_key >= KeySlice::from_slice(k),
        Bound::Excluded(k) => last_key > KeySlice::from_slice(k),
        Bound::Unbounded => true,
    };
    let upper_ok = match upper {
        Bound::Included(k) => first_key <= KeySlice::from_slice(k),
        Bound::Excluded(k) => first_key < KeySlice::from_slice(k),
        Bound::Unbounded => true,
    };
    lower_ok && upper_ok
}
